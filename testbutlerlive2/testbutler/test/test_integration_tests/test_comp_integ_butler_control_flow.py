...
from conftest import adb, emulator, test_butler_service  # noqa
from testbutlerlive.device import Device
from testbutlerlive.devicelog import DeviceLog
from testbutlerlive.application import ServiceApplication
from testbutlerlive.parsing import LineParser, TestButlerCommandParser
...
def install_butler(adb: Callable[[], Device], test_butler_service: Callable[[], str]):
    """
...
class TestButlerControlFlow(object):

    @pytest.mark.parametrize('execution_number', range(10))
    # @pytest.mark.skipif(True, reason="Only for manual checkout.  Other tests will stress test integrated system")
    def test_control_flow(self, adb: Callable[[], Device], install_butler: ServiceApplication, execution_number: int):
        device = adb()
