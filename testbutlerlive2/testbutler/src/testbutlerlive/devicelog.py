...
class DeviceLog(RemoteDeviceBased):
    ...
    async def logcat(self,
                     *options: str,
                     ...
                     allow_process_to_continue: bool=False) -> AsyncGenerator[str, str]:
        """
        ...
        :param future: will be set to create asyncio.subprocess.Process object if client wishes to inspect stderr and
           such.  Client can also wait on these future and timeout if desired
        :param allow_process_to_continue: if True, will not kill the process when client finishes (breaks out of)
           the iterator that is reading this async-generator's output,

           CAUTION: in such a case, the process will keeping running in the background
           until the client explicitly kills the process through the result of the returned future
        ...
        """
        async for line in self.device.execute_remote_cmd_async("logcat", *options, future=future,
                                                               allow_process_to_continue=allow_process_to_continue):
            yield line
