...
class InstrumentationOutputParser(LineParser):
    ...
    def __init__(self, test_reporter: TestListener):
        """
        :param test_reporter: Reporter object to report test status on an on-going basis
        """
        super(InstrumentationOutputParser, self).__init__()
        # internal attributes:
        self._reporter = test_reporter
...
class TestButlerCommandParser(LineParser):
    ...
    Note that `testbutlerlive.logcapture.LogcatTagDemuxer` provides the AsyncLineParser interface, which
    ...
    def __init__(self, service: ServiceApplication, app_under_test: Application,
        ...
        self._method_map = {
            self.SETTINGS_PREFIX: self.process_set_device_setting_cmd,
            self.PROPERTY_PREFIX: self.process_set_property_cmd,
            self.GRANT_PREFIX: self.process_grant_permission_cmd,
        }
    ...
    def process_set_device_setting_cmd(self, cmd):
        """
        ...
        if namespace is None or key is None or value is None:
            log.error("Missing namespace, key or value in command: %s" % cmd)
            return
    ...
    def process_set_property_cmd(self, cmd):
        """
    ...
    def process_grant_permission_cmd(self, cmd):
        """
