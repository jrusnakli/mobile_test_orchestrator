...
from typing import List, Union, TypeVar, Type, AsyncGenerator

from .device import Device, RemoteDeviceBased, TIMEOUT_LONG_ADB_CMD
...
class Application(RemoteDeviceBased):
    ...
    @classmethod
    def _verify_install(cls, appl_path: str, package: str, device: Device) -> int:
        """
    ...
    def _install_common(cls, apk_path: str, device: Device, as_upgrade, package: str) -> None:
        ...
        future_returncode = asyncio.get_event_loop().create_future()
        ...
        async def execute(future: asyncio.Future):
            """
            ...
            async for msg in device.execute_remote_cmd_async(*cmd, future=future):
                log.debug(msg)

                if cls.ERROR_MSG_INSUFFICIENT_STORAGE in msg:
                    future.set_exception(cls.InsufficientStorageError("Insufficient storage for install of %s" %
                                                                      apk_path))
        ...
        async def timer(future):
            """
            :param future: future to wait on for timer
            """
            await asyncio.wait_for(future, TIMEOUT_LONG_ADB_CMD)

        asyncio.get_event_loop().create_task(timer(future_returncode))
        asyncio.get_event_loop().run_until_complete(execute(future_returncode))
        return_code = future_returncode.result()  # will raise Exception if exception raised in coroutine
        if return_code != 0:
            raise Exception("Command to clean-install application failed with status code %s" % return_code)
        else:
    ...
    def start(self, activity: Union[str, None], *options: str, intent: Union[str, None] = None)->None:
        """
...
class ServiceApplication(Application):
    ...
    def start(self, activity: str, *options: str, intent: Union[str, None] = None):
        """
...
class TestApplication(Application):
    ...
    async def run(self, *options: str, future: Union[asyncio.Future, None] = None) -> AsyncGenerator:
        """
        Run an instrumentation test package, yielding lines from std output

        :param options: arguments to pass to instrument command
        :param future: clients can timeout on this as necessary;  set only once all processing is done by client
           or when process ends and there are no more lines of output
        """
        ...
        async for line in self.device.execute_remote_cmd_async("shell", "am", "instrument", "-w", "-r", *options,
                                                               "/".join([self._package_name, self._runner]),
                                                               future=future):
            yield line
        if future:
            # will raise exception if exception encountered during run:
            log.debug("Return code of instrument command: %s" % future.result())
