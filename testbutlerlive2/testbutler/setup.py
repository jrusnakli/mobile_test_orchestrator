...
APK_RESOURCES_DIR = os.path.join(os.path.dirname(__file__), "src", "testbutlerlive", "resources", "apks")
...
def build_signed_app():
    ...
    if sys.platform == 'win32':
        completed = subprocess.run(["tb_gradlew", "assembleRelease"], cwd=BUTLER_SERVICE_SRC_DIR, env=my_env,
                                   shell=True)
    else:
        completed = subprocess.run(["./tb_gradlew", "assembleRelease"], cwd=BUTLER_SERVICE_SRC_DIR, env=my_env)
    if completed.returncode != 0:
